...
st.caption("Carga un archivo Excel, selecciona cilindro/hoja y tipo de cojinete.")
...
def cargar_hojas(file):
    file.seek(0)
    wb = openpyxl.load_workbook(file, data_only=True)
    return wb.sheetnames

def leer_matriz(file, hoja, tipo):
    """
    Estructura usada en tus archivos:
    UPPER: filas 14 a 18, columnas G:I
    LOWER: filas 14 a 18, columnas R:T
    """
    file.seek(0)
    wb = openpyxl.load_workbook(file, data_only=True)
    ws = wb[hoja]
    ...
    filas = range(14, 19)
    columnas = [7, 8, 9] if tipo == "UPPER" else [18, 19, 20]
    ...
    for r in filas:
        ...
        for c in columnas:
            ...
            if value is None:
                raise ValueError(f"Celda vacía en hoja {hoja}, fila {r}, columna {c}")
...
def crear_grafico(arr, equipo, hoja, tipo, horas, supervisor):
    # Escala técnica fija:
    ...
    fig.text(0.04, 0.905, f"Equipo: {equipo}   |   Cilindro/Hoja: {hoja}   |   Tipo: {tipo}",
             fontsize=10, color="#0b55b8")
    fig.text(0.04, 0.88, f"Horas de operación: {horas}   |   Supervisor: {supervisor}",
             fontsize=10, color="#0b2545")
    ...
    fig.text(0.08, 0.135, "<0.90 SEVERO", fontsize=8, color="#b30000", fontweight="bold")
    fig.text(0.19, 0.135, "0.90-0.99 DESGASTE", fontsize=8, color="#f57c00", fontweight="bold")
    fig.text(0.34, 0.135, "≥1.20 NORMAL / EXCELENTE", fontsize=8, color="#168323", fontweight="bold")

    return fig

if archivo:
    hojas = cargar_hojas(archivo)

    c1, c2 = st.columns(2)
    with c1:
        hoja = st.selectbox("Seleccionar cilindro / hoja", hojas)
    with c2:
        tipo = st.selectbox("Tipo de cojinete", ["UPPER", "LOWER"])

    if st.button("Generar gráfico", type="primary"):
        try:
            arr = leer_matriz(archivo, hoja, tipo)
            fig = crear_grafico(arr, equipo, hoja, tipo, horas, supervisor)
            ...
            st.download_button(
                "Descargar PNG",
                data=png.getvalue(),
                file_name=f"grafico_{hoja}_{tipo}.png",
                mime="image/png"
            )
            ...
            st.download_button(
                "Descargar PDF",
                data=pdf.getvalue(),
                file_name=f"grafico_{hoja}_{tipo}.pdf",
                mime="application/pdf"
            )

        except Exception as e:
            st.error(f"No se pudo generar el gráfico: {e}")
            st.info("Verifica que la hoja seleccionada tenga la estructura original: UPPER en G:I filas 14-18 y LOWER en R:T filas 14-18.")
else:
