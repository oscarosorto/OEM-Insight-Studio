...
.title { color:#0b2545; font-size:34px; font-weight:800; }
...
def create_report(data, meta):
    ...
    ax_logo = fig.add_axes([0.025, 0.905, 0.165, 0.080])
    ax_logo.axis("off")
    if logo_path.exists():
    ...
    fig.text(
        ...
        fontsize=22, fontweight="bold", color="#0b2545",
        ha="left", va="center"
    )
    ...
    fig.text(
        ...
        fontsize=11.2, color="#0b55b8", ha="left", va="center"
    )

    ax_meta = fig.add_axes([0.835, 0.885, 0.145, 0.110])
    ax_meta.axis("off")
    ax_meta.add_patch(FancyBboxPatch((0, 0), 1, 1, boxstyle="round,pad=0.015",
    ...
    ax_eq.axis("off")
    ax_eq.add_patch(FancyBboxPatch((0, 0), 1, 1, boxstyle="round,pad=0.008",
                                   facecolor="white", edgecolor="#c9d4e4", linewidth=0.8))
    ...
    for label, value, width, dark in fields:
        if dark:
            ax_eq.add_patch(FancyBboxPatch((x, 0.12), width, 0.76, boxstyle="round,pad=0.006",
                                           facecolor="#0b2545", edgecolor="#0b2545"))
            color = "white"
        else:
            ax_eq.add_line(plt.Line2D([x, x], [0.15, 0.85], color="#c9d4e4", linewidth=1))
            color = "#0b2545"
    ...
    def draw_panel(title, arr, x, y, w, h):
        ...
        ax_box.axis("off")
        ax_box.add_patch(FancyBboxPatch((0, 0), 1, 1, boxstyle="round,pad=0.006",
        ...
        ax_map.imshow(Z.T, origin="lower", extent=[1, 5, 1, 3], cmap=cmap, norm=norm,
        ...
        ax_map.scatter(mi[0] + 1, mi[1] + 1, s=70, marker="s",
        ...
        for idx, (label, value, color) in enumerate(kpis):
            ...
            fig.add_artist(FancyBboxPatch((kpi_x, yy), kpi_w, h * 0.120,
                                          transform=fig.transFigure,
                                          boxstyle="round,pad=0.003,rounding_size=0.004",
                                          facecolor="#f8fafc", edgecolor="#d7e0ec", linewidth=0.55))
            fig.text(kpi_x + kpi_w / 2, yy + h * 0.078, label,
                     fontsize=5.8, color=color, ha="center", fontweight="bold")
            fig.text(kpi_x + kpi_w / 2, yy + h * 0.028, f"{value:.2f}",
                     fontsize=10, color=color, ha="center", fontweight="bold")
            fig.text(kpi_x + kpi_w * 0.78, yy + h * 0.032, "mm",
                     fontsize=5.2, color=color, ha="left", fontweight="bold")
        ...
        cond_w = w * 0.225
        cond_h = h * 0.125
        fig.add_artist(FancyBboxPatch((cond_x, cond_y), cond_w, cond_h,
                                      transform=fig.transFigure,
                                      boxstyle="round,pad=0.004,rounding_size=0.006",
                                      facecolor=cond_color, edgecolor=cond_color, linewidth=0.7))

        fig.add_artist(Circle((cond_x + cond_w * 0.18, cond_y + cond_h * 0.50),
                              cond_h * 0.23,
                              transform=fig.transFigure,
                              facecolor="none",
        ...
        fig.text(cond_x + cond_w * 0.62, cond_y + cond_h * 0.65,
                 "CONDICIÓN", fontsize=6.2, color=txt_color, ha="center", fontweight="bold")
        fig.text(cond_x + cond_w * 0.62, cond_y + cond_h * 0.31,
                 cond_label, fontsize=9, color=txt_color, ha="center", fontweight="bold")
    ...
    # -------------------------
    # Pie de reporte: escala + simbología + riesgo + diagnóstico
    # -------------------------
    footer_y = 0.055
    footer_h = 0.135

    # Escala de espesor RESTAURADA con valores numéricos
    ax_scale = fig.add_axes([0.025, footer_y, 0.290, footer_h])
    ax_scale.axis("off")
    ax_scale.add_patch(FancyBboxPatch((0, 0), 1, 1, boxstyle="round,pad=0.006",
    ...
    cax = ax_scale.inset_axes([0.07, 0.14, 0.055, 0.68])
    ...
    cax.set_yticklabels([f"{v:.2f}" for v in tick_vals], fontsize=6.5, color="#0b2545")
    ...
    ax_sym.axis("off")
    ax_sym.add_patch(FancyBboxPatch((0, 0), 1, 1, boxstyle="round,pad=0.006",
                                    facecolor="white", edgecolor="#d7e0ec", linewidth=0.8))
    ...
    ax_risk.axis("off")
    ax_risk.add_patch(FancyBboxPatch((0, 0), 1, 1, boxstyle="round,pad=0.006",
                                     facecolor="white", edgecolor="#d7e0ec", linewidth=0.8))
    ...
    ax_diag.axis("off")
    ax_diag.add_patch(FancyBboxPatch((0, 0), 1, 1, boxstyle="round,pad=0.006",
                                     facecolor="white", edgecolor="#d7e0ec", linewidth=0.8))
